...
labels, calibrate thresholds, call an LLM, update a graph, or emit POC events.
...
omitted, IDs `v1`, `v2`, … are assigned in CSV order. Optional `parent_id`
points to exactly one other verified ID in the same CSV. Leave it blank for a
root party. Optional `aliases` contains `|`-separated, explicitly supplied
names. No aliases are inferred. Supplied aliases are treated as authoritative
names, so review them before handoff. Parent cycles, unknown parents and
...
1. **Verified hierarchy — `graph.py`.** Each verified row becomes one node.
   `parent_id` is the single parent pointer. `root_id` walks to the ultimate
   parent and detects cycles. The match result is the root verified party,
   even if a child name supplied the evidence. Official names and supplied
   aliases are indexed; duplicate name ownership is tracked for collision
   checks. No graph is persisted or updated. The inherited
   `ExpansionCandidate` data class represents only a supplied CSV alias here;
   this package performs no name expansion.
2. **Mentions and retrieval — `domain.py`, `matching.py`.** An unverified name
   ...
   reciprocal-rank fusion, and caps retained roots and variants. This reverse
   index mirrors the POC batch algorithm; it is rebuilt for each invocation.
...
4. **Root decision — `matching.py`.** Proposals from a child and its parent
   are grouped by root; only the strongest variant per root competes. The
   fixed rules target score factors in the gap from the runner-up. Digit and
   distinctive-token conflicts can veto a non-exact match. A close second
   root triggers ambiguity abstention. Plain names use a 0.800 cutoff;
   connector mentions ordinarily use a 0.933 cutoff. A guarded
   ...
   characters uniquely identifies one verified root, is present in its
...
5. **OBO/VIA — `matching.py`.** Each part gets its own candidates and score.
   ...
   VIA when they point to different roots. If the preferred part is just
...
6. **Enhanced recovery — `rules_enhancement.py`.** This runs only on rejected
   ...
   recovery may accept a verified-root-unique short name,
   safely score a trimmed/spacing/plural name view, or recognize a complete
...
calibration data are needed. The verified CSV replaces the POC's verified
graph and expansion cache. Only explicitly supplied aliases and parent links
are available. With different verified/unverified populations, TF-IDF and
token-frequency statistics change naturally, so scores and predictions may
...
run. `../scripts/build_rules_handoff.py` documents how selected runtime
functions were mechanically extracted from the parent POC for future syncs;
recipients do not need that script.
