...
The rules-only workbook's Stats sheet also reports **all labeled recall excluding the workbook `ERROR` category**. This second denominator includes scorable TRAIN, CALIBRATION, TEST_KNOWN, and TEST_UNSEEN rows; `UNKNOWN` and other unscorable rows are excluded, while `NO_MATCH` counts as a miss. It is a descriptive result on all labeled data, not an independent held-out quality estimate. The original held-out metrics remain unchanged.
...
Within that second pass, `decision.rules_identity_tiebreak_enabled = true` adds narrow plain-name recoveries: full official-name `&`/`and` equivalence, optional leading `The`, `Labs`/`Laboratories` abbreviation when it identifies one verified root, and an explicit legal-form tie-break when the raw name itself says, for example, `Inc` rather than `LLC`. The stated form must identify one verified root, and a competing organization or equivalent-form duplicate still causes abstention. `Abbott Molecular` and `Abbott Nutrition` are not abbreviations of `Abbott Laboratories`; the matcher does not infer ownership from a shared brand. Bare names such as `Sodexo` do not select `Sodexo, Inc.` by assumption. The flag can be set to `false` to compare with the previous second pass; it does not alter global normalization, retrieval indexes, training artifacts, accepted decisions, or connectors. Re-run matching (not just `report_rules.py`) to see the new decisions and route counts in the rules-only workbook. Compare fresh runs with the flag on and off for its net precision/recall effect; `rules_second_pass_comparison.json` measures the whole second pass, not this route alone.
...
`decision.rules_multipart_enabled = true` adds an optional **rules-only** selector for plain names with strong, competing verified matches in separate dash/pipe/parenthesis fragments. It searches and scores each fragment independently, preserves a strong exact full-name match, and makes one choice using score plus a small delimiter-position prior estimated from TRAIN/CALIBRATION labels only. No company-specific alias or verified ID is hardcoded; TEST labels are not used to choose. The output method is `rules:multipart_forced_choice` so its decisions can be audited. `rules_pre_multipart_decisions.jsonl` preserves the complete earlier enhanced output and `rules_multipart_comparison.json` compares held-out precision/recall/F1 before and after this stage. Set the flag to `false` and rerun to restore the earlier rules result. This stage may reduce precision: a score measures name similarity, not which party a multi-entity row intends, and the prior is a weak heuristic. It does not change ML decisions, emitted events, connector handling, or `rules_only_handoff/`.

`decision.rules_verified_anchor_enabled = true` runs a separate, final rules-only experiment on remaining plain `NO_MATCH` rows. It looks up complete official-name prefixes and verified names with a terminal legal form omitted (including bank `N.A.`), rather than raising a global string-similarity weight. A candidate anchor must identify one verified root; a more specific official name from another root or an exact whole-name competitor blocks it. Context-bearing short anchors require an uppercase acronym, while a bare short base such as `Ripe` may match a unique `Ripe Limited` root. This is a name heuristic, not proof that a branch or department is legally the same entity. It assigns capped heuristic confidence (0.95 for a complete official-name prefix, 0.90 for a base-name route) and respects the plain and request cutoffs. Set the flag to `false` to restore the complete pre-anchor enhanced result. `rules_pre_anchor_decisions.jsonl`, `rules_anchor_comparison.json`, and `rules_anchor_changes.jsonl` preserve the before/after decisions, held-out metrics, and each changed row. The final `rules_decisions.jsonl` and rules-only demo workbook include this stage. No ML decision, event, connector, or frozen handoff code is changed.
